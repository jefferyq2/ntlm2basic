...
build: $(SRCS)
	...
	CGO_ENABLED=0
ifeq ($(GOOS),darwin)
	go build -o bin/ntlm2basic $(DARWIN_OPTS) cmd/$(NAME)/main.go
endif
ifeq ($(GOOS),linux)
	go build -o bin/ntlm2basic $(LINUX_OPTS) cmd/$(NAME)/main.go
endif
ifeq ($(GOOS),windows)
	go build -o bin/ntlm2basic.exe $(WINDOWS_OPTS) cmd/$(NAME)/main.go
endif
